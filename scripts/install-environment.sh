...
# Check if the host is osx-arm64 or osx-64
is_osx() {
  [[ "$(uname -s)" == "Darwin" && ( "$(uname -m)" == "arm64" || "$(uname -m)" == "x86_64" ) ]]
}
...
install-pixi-global() {
  ...
  pixi global install \
    git gh coverage pdoc pre-commit ruff google-cloud-sdk
}
...
install() {
  ...
  if is_osx; then
    info "osx-arm64 or osx-64 detected: Skipping system package upgrade and reboot."
  else
    upgrade-system-packages
    prompt_reboot
  fi
